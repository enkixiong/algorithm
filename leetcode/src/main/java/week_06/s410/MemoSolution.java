...
public class MemoSolution {
    ...
    int[] info;
    // 备忘录
    // 在强制DFS时的根本性的原因是: 尝试获取最优解的耗时太长,耗时太长的根本性原因是: 已经计算过的情况在重复计算
    public int splitArray(int[] nums, int m) {
        ...
        // 定义状态: 题解的状态是: 以0为起点的数组,大小为m的子数组, 各子数组和之间的最小值
        // 分治: 从nums划分一个子数组,如果该数组,记录当前数组的总和, 并且递归求解剩余 m-1 个子数组中的最小值, 求解当前情况的最小值
        int[][] memo = new int[nums.length][m];
        for(int[] sub : memo){
            Arrays.fill(sub, Integer.MAX_VALUE);
        }
        return dfs(0,0,m,nums,memo);
    }

    private int dfs(int beginIndex, int arrayIndex, int m, int[] nums, int[][] memo) {

        // 备忘录
        if(memo[beginIndex][arrayIndex] != Integer.MAX_VALUE){
            return memo[beginIndex][arrayIndex];
        }

        // terminator, 最后一个元素
        if(arrayIndex == m-1){
            memo[beginIndex][arrayIndex] = info[nums.length] - info[beginIndex];
            return memo[beginIndex][arrayIndex];
        }


        // 从begin/arrayIndex开始, 计算结果
        memo[beginIndex][arrayIndex] = Integer.MAX_VALUE;
        int endIndex = nums.length - m + arrayIndex;
        int current = 0;
